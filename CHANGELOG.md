...
- docs(diagrams): `make diagrams-png` — renders a PNG alongside every diagram SVG via headless Chromium (Playwright), reproducing each SVG's baked-in physical print size crisply (default 300 DPI); `scripts/render_diagram_pngs.py` deliberately doesn't use `cairosvg`, which ignores PlantUML's `textLength`/`lengthAdjust="spacing"` and overflows every class-diagram box's text (#842)
- docs(ch08): `examples/inspector_demo.py` (Agent Inspector quickstart entrypoint, no new dependency — the Inspector CLI loads it externally as config) and `scripts/capture_inspector_screenshots.py` (Python Playwright, drives a real session against a live Ollama backend, captures all three manuscript screenshots at 2x device scale) replace the old low-resolution manual screenshots; `scripts/wrap_png_as_svg.py` wraps each PNG in an SVG with a fixed physical size baked in (#841)
- docs(ch08): UML sequence diagram for the Result Approval Gate (Figure 8.5); `scripts/fix_svg_background.py` (PlantUML's newer SVG output relies on a CSS background hint many viewers ignore), `scripts/add_svg_legend.py` (numbered legend overlaid directly on the rendered SVG, since PlantUML's own `legend`/`note` can't be pinned to an arbitrary spot without disturbing layout), and `scripts/set_svg_print_size.py` (bakes a fixed 9pt-target/5.6in x 7in-capped physical size into every diagram SVG, replacing ad hoc manual Canva resizing) (#838)

### Changed

- chore: rename `scripts/` to `_scripts/` (#843)
