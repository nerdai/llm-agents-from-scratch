...
- chore: `mcp[cli]` lower bound `>=1.9.4` → `>=2.0.0` in `pyproject.toml` — the code has required mcp's 2.0.0+ API (`streamable_http_client`, renamed from `streamablehttp_client`) since #742, but the dependency pin was never bumped to match; today's resolution already lands on mcp 2.x, but nothing previously enforced it (#863)
