...
- fix(ch10): `LLMAgentA2AExecutor.cancel()`'s ordering comment was stale on `a2a-sdk` 1.1.2 — it attributed the risk to our own `task_handler.cancel()` waking `execute()`, but `ActiveTask.cancel()` actually cancels the producer task itself before calling us, making that step dead code on the default request handler's path; rewrote the comment (and `StreamingLLMAgentA2AExecutor`'s cross-reference) to state the real invariant — nothing that can `await` may precede `updater.cancel()`, or the already-cancelled producer's `finally` closes the event queue first and the terminal status is silently dropped, deterministically, not as a race. Added `tests/a2a/server/test_executor_cancel_regression.py`, driving the real SDK request-handler stack end-to-end (the existing tests call `cancel()` against a bare queue that never closes, so they pass under any ordering, including a broken one) (#885)
